...
import { getFashionJewellary, addFashionJewellary, deleteFashionJewellary, editFashionJewellary } from '../../api/api'
import { toast } from 'sonner'

const AdminFashionJewellary = () => {
  ...
  //new
  const [currentFashionJewellary, setCurrentFashionJewellary] = useState(null)
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await addFashionJewellary(FashionJewellary)
  ...
  const editHelper = (FashionJewellary) => {
    console.log(FashionJewellary)
    setCurrentFashionJewellary(FashionJewellary)
    setShowEdit(true)
  }
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editFashionJewellary(FashionJewellary, currentFashionJewellary._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteFashionJewellary(id)
      if (response.status === 200) {
  ...
  return (
    <div className='w-full flex flex-col justify-start items-start'>
      <div className='w-full flex flex-row justify-between items-center my-4 shadow-md rounded-md p-1 border'>
        <AdminPageHeader title='FashionJewellary' />
        ...
        <tbody>

          {
            FashionJewellary.map((FashionJewellary, index) => (
              <tr key={index}>
                <td className='p-4'>{FashionJewellary._id} </td>
                <td className='flex justify-start px-4 items-center'><img src={FashionJewellary.img} alt={FashionJewellary.title} className='h-12 w-12 object-cover rounded-full shadow-md bg-purple-500' /></td>
                <td className='p-4'>{FashionJewellary.title} </td>
                <td className='p-4'>{FashionJewellary.price}</td>
                <td className='p-4'>{FashionJewellary.brand}</td>
            ...
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                ...
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-8' onSubmit={handleEdit}>
                  <input ref={titleRef} type="text" name="" id="title" placeholder='Title' defaultValue={currentFashionJewellary.title} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required autoFocus />
                  <input ref={imgRef} type="text" name="" id="img" placeholder='Image URL' defaultValue={currentFashionJewellary.img} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={priceRef} type="text" name="" id="price" placeholder='Price' defaultValue={currentFashionJewellary.price} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={brandRef} type="text" name="" id="brand" placeholder='brand' defaultValue={currentFashionJewellary.brand} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
...
export default AdminFashionJewellary