...
import { getWeddingJewellary, addWeddingJewellary, deleteWeddingJewellary, editWeddingJewellary } from '../../api/api'
import { toast } from 'sonner'

const AdminWeddingJewellary = () => {
  ...
  //new
  const [currentWeddingJewellary, setCurrentWeddingJewellary] = useState(null)
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await addWeddingJewellary(WeddingJewellary)
  ...
  const editHelper = (WeddingJewellary) => {
    console.log(WeddingJewellary)
    setCurrentWeddingJewellary(WeddingJewellary)
    setShowEdit(true)
  }
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editWeddingJewellary(WeddingJewellary, currentWeddingJewellary._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteWeddingJewellary(id)
      if (response.status === 200) {
        ...
        <tbody>

          {
            WeddingJewellary.map((WeddingJewellary, index) => (
              <tr key={index}>
                <td className='p-4'>{WeddingJewellary._id} </td>
                <td className='flex justify-start px-4 items-center'><img src={WeddingJewellary.img} alt={WeddingJewellary.title} className='h-12 w-12 object-cover rounded-full shadow-md bg-purple-500' /></td>
                <td className='p-4'>{WeddingJewellary.title} </td>
                <td className='p-4'>{WeddingJewellary.price}</td>
                <td className='p-4'>{WeddingJewellary.brand}</td>
...
export default AdminWeddingJewellary