...
import { getBeedJewellary, addBeedJewellary, deleteBeedJewellary, editBeedJewellary } from '../../api/api'
import { toast } from 'sonner'

const AdminBeedJewellary = () => {
  ...
  //new
  const [currentBeedJewellary, setCurrentBeedJewellary] = useState(null)
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await addBeedJewellary(BeedJewellary)
  ...
  const editHelper = (BeedJewellary) => {
    console.log(BeedJewellary)
    setCurrentBeedJewellary(BeedJewellary)
    setShowEdit(true)
  }
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editBeedJewellary(BeedJewellary, currentBeedJewellary._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteBeedJewellary(id)
      if (response.status === 200) {
  ...
  return (
    <div className='w-full flex flex-col justify-start items-start'>
      <div className='w-full flex flex-row justify-between items-center my-4 shadow-md rounded-md p-1 border'>
        <AdminPageHeader title='BeedJewellary' />
        ...
        <tbody>

          {
            BeedJewellary.map((BeedJewellary, index) => (
              <tr key={index}>
                <td className='p-4'>{BeedJewellary._id} </td>
                <td className='flex justify-start px-4 items-center'><img src={BeedJewellary.img} alt={BeedJewellary.title} className='h-12 w-12 object-cover rounded-full shadow-md bg-purple-500' /></td>
                <td className='p-4'>{BeedJewellary.title} </td>
                <td className='p-4'>{BeedJewellary.price}</td>
                <td className='p-4'>{BeedJewellary.brand}</td>
               ...
               hover:bg-blue-500 hover:text-white hover:shadow-blue-500'
                    onClick={() => { editHelper(BeedJewellary) }}>
                    <Pencil />
                  </button>
                  <button className='h-15 w-15 border-purple-500 border-2 p-1 rounded-md text-purple-500 shadow-md
               hover:bg-red-500 hover:text-white hover:shadow-purple-500'
                    onClick={() => { handleDelete(BeedJewellary._id) }}>
                    <Trash />
                  </button>
                </td>
              </tr>
            ...
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  ...
                  <div className="w-1/2 flex justify-end items-center text-product-500 cursor-pointer" onClick={() => { setShowEdit(!showEdit) }}>
                    <X className="h-8 w-8 border-2 p-1  border-purple-500 rounded-full  hover:bg-purple-500 hover:text-white" />
                  </div>
                </div>
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-8' onSubmit={handleEdit}>
                  <input ref={titleRef} type="text" name="" id="title" placeholder='Title' defaultValue={currentBeedJewellary.title} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required autoFocus />
                  <input ref={imgRef} type="text" name="" id="img" placeholder='Image URL' defaultValue={currentBeedJewellary.img} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={priceRef} type="text" name="" id="price" placeholder='Price' defaultValue={currentBeedJewellary.price} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={brandRef} type="text" name="" id="brand" placeholder='brand' defaultValue={currentBeedJewellary.brand} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
...
export default AdminBeedJewellary